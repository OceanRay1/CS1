...
class CS1Canvas(QWidget):
    ...
    def draw_point(self, x, y):
        self.ipainter.drawPoint(int(x), int(y))

    def draw_line(self, x1, y1, x2, y2):
        self.ipainter.drawLine(int(x1), int(y1), int(x2), int(y2))

    def draw_rectangle(self, x, y, w, h):
        self.ipainter.drawRect(int(x), int(y), int(w), int(h))

    def draw_polygon(self, vertices):
        qpoints = []
        for vertex in vertices:
            # Cast each coordinate to int
            qpoints.append(QPoint(int(vertex[0]), int(vertex[1])))
        poly = QPolygonF(qpoints)
        self.ipainter.drawPolygon(poly)

    def draw_ellipse(self, x, y, rx, ry):
        # QRectF can accept floats, but to be consistent cast here:
        rect = QRectF(float(x) - float(rx), float(y) - float(ry), float(rx) * 2, float(ry) * 2)
        self.ipainter.drawEllipse(rect)
    ...
    def draw_image(self, image, x, y):
        # Usually QPainter.drawImage(x, y, image) accepts ints,
        # so cast here
        self.ipainter.drawImage(int(x), int(y), image)
